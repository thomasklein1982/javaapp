...
\befehlJA{addButtonListener(String b, (ev)->{Anweisungen})}{Legt fest, welche Anweisungen ausgeführt werden sollen, wenn der entsprechende Button gedrückt wird. Mögliche Werte für \mono{b} sind \injava{"A"}, \injava{"B"}, \injava{"X"}, \injava{"Y"}, \injava{"UP"}, \injava{"DOWN"}, \injava{"LEFT"}, \injava{"RIGHT"}. 
%\getbuffer[addbuttonlistener]
}
\befehlJA{setBounds(double x, double y, double w, double h)}{Legt fest, in welchem Bereich des Bildschirms das Gamepad angezeigt werden soll. Alle Angaben sind in \percent. $(\mono{x}\mid \mono{y})$ ist der linke untere Eckpunkt, \mono{w} ist die Breite, \mono{h} die Höhe.}
\stopitemize
\stopsubject
