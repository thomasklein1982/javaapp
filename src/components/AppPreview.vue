    ...
    methods: {
      ...
      reload(noDebugging, args, additionalCode){
        ...
        if(this.$root.webMode){
          ...
          prefix=noDebugging?"$App.console.hide();":"$App.debug.setBreakpoints("+JSON.stringify(this.breakpoints)+"); window.addEventListener('keydown',(e)=>{let key=e.code||e.keyCode; if ( key === 113 || key==='F2') {e.preventDefault();$Exercise.sendMessage('keydown',{key: 'F2'});}},true);";
        }
